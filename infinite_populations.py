    ...
    def plot_phase_diagram(N, T, resolution_cost = 100):
        ...
        for c_d in c_d_values:
            for c_h in c_h_values:
                rep_dyn = InfiniteNPlayerHDGTDynamics(N, c_h, nb_states=100, T=T, c_d=c_d)
                ...
                if (not already_labeled[phase]):
                    ...
                    plt.plot([c_h], [c_d], label = phase_dictionary[phase][0],marker='.', markersize=10, color=color)
                else:
                    plt.plot([c_h], [c_d], marker='.', markersize= 500/resolution_cost, color=color)
        plt.title(f'T = {T}')
        plt.legend()
        plt.show()
